...
_REACT_PROMPT_DEFAULTS: dict[str, str] = {
    "react_expense_analysis": (
        ...
        "  - final_answer: Call this when you have completed your analysis.\n\n"
        "Workflow:\n"
        ...
        "  4. Finally call final_answer with a complete summary.\n\n"
        "Guidelines:\n"
        "  - If there are no transactions to process, call final_answer directly.\n"
        "  - Set needs_hitl_confirmation=True in final_answer since the results need "
        "user approval before they are used by other agents.\n"
    ...
    "react_budget_planning": (
        ...
        "Available tools:\n"
        "  - extract_budget_request_tool: Parse the user's budget request to extract "
        "monthly income and categories of interest.\n"
        ...
        "  - final_answer: Call this when the budget plan is complete.\n\n"
        "Workflow:\n"
        "  1. Start by calling extract_budget_request_tool to understand the user's needs.\n"
        "  2. If the extractor says monthly income is missing, use final_answer with "
        "needs_hitl_confirmation=True to ask the user.\n"
        "  3. Generate allocations using generate_allocations_tool with the spending data "
        "provided in context.\n"
        "  4. Calculate actual spending with calculate_spending_tool.\n"
        "  5. Evaluate progress with evaluate_progress_tool.\n"
        "  6. Generate warnings with generate_warnings_tool.\n"
        "  7. Validate with validate_budget_tool. If invalid, fix and re-validate.\n"
        "  8. Call final_answer when everything is valid.\n\n"
    ...
    "react_goal_planning": (
        ...
        "Available tools:\n"
        "  - extract_goal_tool: Extract structured goal information from the user's "
        "natural-language message.\n"
        "  - create_goal_tool: Create a new FinancialGoal with validated parameters.\n"
        "  - calculate_required_saving_tool: Calculate monthly saving needed for a goal.\n"
        "  - final_answer: Call this when done.\n\n"
        "Workflow:\n"
        "  1. Call extract_goal_tool to parse the user's goal request.\n"
        "  2. If the extraction shows missing fields (missing_fields non-empty), "
        "call final_answer with needs_hitl_confirmation=True so the user can "
        "provide the missing information.\n"
        "  3. If the user has provided enough information, call create_goal_tool "
        "to create the goal.\n"
        "  4. Call calculate_required_saving_tool to see if the goal is achievable.\n"
        "  5. Call final_answer with the results.\n\n"
        "Context:\n"
        "  - Today: {today}\n"
